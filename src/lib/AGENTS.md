...
- `pricing.ts`: unit rules plus a guest service fee of 5% through $100, 4% through $500, 3% above $500, capped at $25. Operator payouts take a separate flat 5% (`OPERATOR_FEE_RATE`). `priceUnclaimed` for catalog operators.
...
- `search.ts`: guest search for Explore and the desktop home. One inverted word index built once per catalog (`warmSearch` folds it in during idle time), then ranking by name, activity synonyms, words and city. Stems, typos and the operator name picker (`searchByName`) all read that index. Scope by city or category through `SearchScope`, never by passing a freshly filtered array, or the index rebuilds every keystroke. `searchSuggest` returns the grouped typeahead and the feed in one pass. No invented listings. An occasion ("date night", "tonight", "team building") ranks by the kind order in `src/data/synonyms.ts` (the order is the ranking, so keep day trips like `tour` last for dates), a listing that names the occasion itself comes first, an evening query demotes day-long trips and places whose published hours end by six, and a partner's product sits behind Outset's own operator of the same fit.

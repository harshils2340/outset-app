...
// The rung is how many weekdays this job has actually sent on, not the calendar date and not the launches:
// a weekend, an outage or a day with no headroom left under the ceiling never moves it. See ramp.ts.
...
console.log(`outreach-ramp: day ${day} of the ramp (first run ${state.firstDay || today}), ramp says ${rampLimit}, sending up to ${limit}`);
...
saveState(recordRun(state, today, result.sent));

 ...
 *
 *   npx tsx scripts/landing-pages-dry.mts [out-dir] [art] [metro]
 */
...
console.log(`${items.length} listings -> ${r.pages} pages (${r.metroPages} city pages, ${r.kindPages} all-metro pages) plus index in ${Date.now() - t0} ms, ${(bytes / 1024 / 1024).toFixed(1)} MB total, ${Math.round(bytes / files.length / 1024)} KB average`);
console.log(`sitemap: ${(readFileSync(join(out, "sitemap.xml"), "utf8").match(/<loc>/g) || []).length} urls`);
console.log(`out: ${out}`);

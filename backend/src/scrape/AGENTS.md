...
Headless Chrome is a last resort (`render.ts`). Never `unref()` the process. Tag profiles `/tmp/outset-render-`. Exit, Ctrl+C, and the next backend command must kill leftovers. Do not leave Playwright running after a crawl. `npm run chrome:reap` if Activity Monitor still shows `chromium_headless_shell`.

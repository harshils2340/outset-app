...
2. Set these environment variables on the service:
   ...
   - `GITHUB_TOKEN`: a fine-grained GitHub token, repository `outset-app`, permission Contents: read and write. It publishes the scrubbed guest copy of each claimed profile (`public/profiles/<id>.json`) so the site rebuilds on an edit. Nothing private is written to the repo.
